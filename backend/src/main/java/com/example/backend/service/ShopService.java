...
public class ShopService {
    ...
    public Color buyColor(Account account, ColorResponse colorResponse) throws InstantiationException {
        List<Color> colors = colorRepository.findColorsByAccountId(account.getId());
        if (colors.stream().anyMatch(c -> c.getId().equals(colorResponse.getId()))) {
            throw new InstantiationException("You already own this color");
        }

            Optional<Color> color = colorRepository.findById(colorResponse.getId());
            if (color.isPresent()) {
    ...
    public List<ColorResponse> getAvailableColors() {
        List<Color> colors = colorRepository.findAll();
        List<ColorResponse> colorResponses = new ArrayList<>();
        for (Color color : colors) {
            ColorResponse colorResponse = new ColorResponse(color);
